...
Status S3FileSystem::list_impl(const Path& dir, bool only_file, std::vector<FileInfo>* files,
    ...
    *exists = true;
    ...
    std::unique_ptr<ObjectListIterator> list_iter = client->list_objects(
            ...
            {.endpoint = _client->s3_client_conf().endpoint});
    ...
    do {
        auto list_result = rate_limit_next_func();
        if (list_result.resp.status.code != TStatusCode::OK) {
            return Status(list_result.resp.status.code, std::move(list_result.resp.status.msg))
                    .append(fmt::format("failed to list objects with prefix {}",
                                        full_s3_path(prefix)));
        }
        auto obj = list_result.results_;
        obj.value().file_path.erase(0, prefix.size());
    } while (true);
...
std::string S3FileSystem::generate_presigned_url(const Path& path, int64_t expiration_secs,
    ...
    return client->generate_presigned_url(
            ...
            {.endpoint = s3_conf.endpoint, .ak = s3_conf.ak, .sk = s3_conf.sk}, expiration_secs);
}
