...
public:
    ...
    ObjectStorageUploadResponse create_multipart_upload(
            const ObjectStoragePathOptions& opts, const ObjectClientConfigOptions& config) override;
    ...
    std::unique_ptr<ObjectListIterator> list_objects(
            const ObjectStoragePathOptions& path, const ObjectClientConfigOptions& config) override;
