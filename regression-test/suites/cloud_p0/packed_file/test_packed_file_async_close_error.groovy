...
suite("test_packed_file_async_close_error", "p0, nonConcurrent") {
    ...
    setBeConfigTemporary([
        ...
        try {
            ...
            for (int i = 0; i < 30; ++i) {
                try {
                    sql """ SELECT COUNT(*) FROM test_packed_file_async_close_error """
                    sleep(1000)
                } catch (Throwable t) {
                    queryFailed = true
                    queryError = t.getMessage()
                    logger.info("query failed after injected packed file upload error: ${queryError}")
                    break
                }
            }
            assertTrue(queryFailed, "query should fail after packed file metadata is committed but object upload fails")
        } finally {
            GetDebugPoint().disableDebugPointForAllBEs(closeErrorPoint)
            GetDebugPoint().disableDebugPointForAllBEs(afterFinalizeSleepPoint)
            GetDebugPoint().disableDebugPointForAllBEs(waitDatClosedPoint)
