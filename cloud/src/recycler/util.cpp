...
std::string job_log_context(const std::string& instance_id, std::string_view unit_id) {
    if (unit_id.empty()) {
        return fmt::format(" instance={}", instance_id);
    }
    return fmt::format(" instance={} unit={}", instance_id, unit_id);
}

int prepare_recycle_job(TxnKv* txn_kv, std::string_view key, const std::string& instance_id,
                        std::string_view unit_id, const std::string& ip_port, int64_t interval_ms) {
    std::string val;
    std::unique_ptr<Transaction> txn;
    TxnErrorCode err = txn_kv->create_txn(&txn);
    if (err != TxnErrorCode::TXN_OK) {
        LOG(WARNING) << "failed to create txn" << job_log_context(instance_id, unit_id);
        return -1;
    }
    err = txn->get(key, &val);
    if (err != TxnErrorCode::TXN_OK && err != TxnErrorCode::TXN_KEY_NOT_FOUND) {
        LOG(WARNING) << "failed to get kv, err=" << err << " key=" << hex(key)
                     << job_log_context(instance_id, unit_id);
        return -1;
    }
...
void finish_recycle_job(TxnKv* txn_kv, std::string_view key, const std::string& instance_id,
                        std::string_view unit_id, const std::string& ip_port, bool success,
                        int64_t ctime_ms) {
    std::string val;
    int retry_times = 0;
    do {
        ...
        if (job_info.ip_port() != ip_port) {
            LOG(WARNING) << "job is doing at other machine: " << job_info.ip_port()
                         << " key=" << hex(key) << job_log_context(instance_id, unit_id);
            return;
        }
        if (job_info.status() != JobRecyclePB::BUSY) {
        ...
        LOG(WARNING) << "failed to commit to finish recycle job, err=" << err << " key=" << hex(key)
                     << " retry_times=" << retry_times << job_log_context(instance_id, unit_id);
    } while (retry_times++ < 3);
    LOG(WARNING) << "finally failed to commit to finish recycle job, key=" << hex(key)
                 << job_log_context(instance_id, unit_id);
}

int lease_recycle_job(TxnKv* txn_kv, std::string_view key, const std::string& instance_id,
                      std::string_view unit_id, const std::string& ip_port) {
    std::string val;
    std::unique_ptr<Transaction> txn;
    TxnErrorCode err = txn_kv->create_txn(&txn);
    if (err != TxnErrorCode::TXN_OK) {
...
int prepare_instance_recycle_job(TxnKv* txn_kv, std::string_view key,
                                 ...
                                 int64_t interval_ms) {
    return prepare_recycle_job(txn_kv, key, instance_id, "", ip_port, interval_ms);
}
...
int prepare_task_recycle_job(TxnKv* txn_kv, std::string_view key, const std::string& instance_id,
                             ...
                             int64_t interval_ms) {
    return prepare_recycle_job(txn_kv, key, instance_id, unit_id, ip_port, interval_ms);
}

void finish_instance_recycle_job(TxnKv* txn_kv, std::string_view key,
                                 const std::string& instance_id, const std::string& ip_port,
                                 bool success, int64_t ctime_ms) {
    finish_recycle_job(txn_kv, key, instance_id, "", ip_port, success, ctime_ms);
}
...
void finish_task_recycle_job(TxnKv* txn_kv, std::string_view key, const std::string& instance_id,
                             ...
                             int64_t ctime_ms) {
    finish_recycle_job(txn_kv, key, instance_id, unit_id, ip_port, success, ctime_ms);
}

int lease_instance_recycle_job(TxnKv* txn_kv, std::string_view key, const std::string& instance_id,
                               const std::string& ip_port) {
    return lease_recycle_job(txn_kv, key, instance_id, "", ip_port);
}

int lease_task_recycle_job(TxnKv* txn_kv, std::string_view key, const std::string& instance_id,
                           const std::string& unit_id, const std::string& ip_port) {
    return lease_recycle_job(txn_kv, key, instance_id, unit_id, ip_port);
}
